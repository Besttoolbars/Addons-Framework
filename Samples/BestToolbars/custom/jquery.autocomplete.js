...
jQuery.autocomplete = function(input, options) {
	...
	jQueryresults.hide().addClass(options.resultsClass);
	...
	// Add to body element
	jQuery('#toolbar-sample').append(results);
	...
	function moveSelect(step) {
		...
		jQuery(lis[active]).addClass('active').css({
			background:'#318cca',
			color:'#fff',
			cursor:'pointer'
		});

		jQueryinput.val(jQuery(lis[active]).text());
	...
	function parseData(data) {
		...
		for (var i=0; i < data.length; i++) {
			if (data[i]) {
				parsed[parsed.length] = data[i].split(options.cellSeparator);
			}
		}
		return parsed;
	};
	...
	function requestData(q) {
		if( (typeof options.url == "string") && (options.url.length > 0) ){
			...
			framework.extension.fireEvent('autocomplite', {url:makeUrl(q)}, function(){});

			framework.extension.detachEvent("_autocomplite", function(){});
			framework.extension.attachEvent("_autocomplite",function(data)
			{
				data = data.text;
				var istart = data.indexOf("[", data.indexOf("[") + 1);
				data = data.substr(istart, data.indexOf("]", istart) - istart + 1);
